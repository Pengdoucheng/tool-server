...
(function(){
  ...
  // ===== Pomodoro =====
  const s = { // state in memory
    mode: "focus", // 'focus' | 'break' | 'long'
    remain: 25*60, running:false, roundsDone:0, focus:25, brk:5, long:15, rounds:4,
    autoNext: true, todayPomos: 0
  };
  // UI refs
  ...
  function savePomo(){
    const all = load(); all.pomo = { mode:s.mode, remain:s.remain, running:s.running, roundsDone:s.roundsDone,
      focus:s.focus, brk:s.brk, long:s.long, rounds:s.rounds, autoNext:s.autoNext, todayPomos:s.todayPomos,
      today: todayKey()
    };
    save(all);
  }
  ...
  // ===== 碼錶 =====
  const sw = { running:false, startAt:0, elapsed:0 }; // elapsed in ms
